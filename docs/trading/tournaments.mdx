...
- Traders:
Anybody can participate in a tournament by paying the tournament buy-ins. If you win a tournament, you get a share of the prize pool, if you guess wrong you don't lose anything. 

## Tournament Formats:

Tournament formats in No Loss Tournaments are determined by the following parameters:
- Buy-in token
- Reward token 
- Guaranteed rewards
- Rake %

### Same Buy-in and Reward Tokens
In this type, the buy-in and reward tokens types are the same. The rake is set at 100%. 

#### Within this type, there are three sub-types based on the prize structure:

- Maximum Guaranteed Win + 100% Rake:
The prize pool is set at a maximum guaranteed amount.

- No Guaranteed Win + Rake < 100%:
The prize pool is not guaranteed and is built from the tournament buy-ins. The rake is less than 100%.

- Minimum Guaranteed Amount with Rake < 100%:
The prize pool is set at a minimum guaranteed amount, with a rake less than 100%.

### Different Buy-in and Reward Tokens
In Type 2, the buy-in and reward tokens are different. The rake is set at 100%.
...
Reward tokens can be USDC, any ERC-20 token or any NFT.
...
- **Start Price Money:** base fund to start the tournaments and should be transferred into the contract before it starts accepting funds from other participants. In case any of the criteria for the tournament to start is not met - the funds are sent back to the wallet it came from just like any participants.

- **Rake %:** % to be deducted from the ticket price. Default is 100% when a Type 1 tournament is made. The rake is transferred to target wallets only after the tournament ends.
...
- **Rebuy-in:** Players can rebuy play tokens with the minimum buy-in

- **Play tokens:** the play token should be non-transferable/fixed amount transferred on every re-buy
...
- **Ranking:** Done based on the score - only eligible are > 1 score

### End of the tournament:

- Once the tournament has ended the total pool can be distributed based on each participant's score

- **Distribution formula:**
100% of the pool is to be distributed among top n players
Admin can set number of winners and winning % per player. The % should add up to 100%. 
