...
Orders are settled on-demand by the [Pyth Network](https://pyth.network/). Pyth is a network of nodes and global trading venues that makes real-time price data available on-chain. Keepers, execute orders as they are submitted to Buffer's contracts using Pyth prices. 
