...
It adds positive expectancy to the platform, thereby improving the possible outcome for LPs. 
...
#### Max Utilization %
This parameter defines the maximum cumulative trade sizes that can be placed across all markets simultaneously. It sets an upper limit on the total exposure across all markets.
...
## Circuit Breakers:
To further protect LPs from potential risks, Buffer implements a liquidity circuit breaker. 
...
A cooldown timer then begins, during which all trade executions are still paused. 